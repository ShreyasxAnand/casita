...
from app.rules.zoning import adu_eligibility, property_type_from_style
...
class ChecklistContext:
    ...
    # Convenience accessors for sub-designation dicts.
    flood: dict[str, Any] = field(default_factory=dict)
    geohazard: dict[str, Any] = field(default_factory=dict)
    historic: dict[str, Any] = field(default_factory=dict)
    wui: dict[str, Any] = field(default_factory=dict)
    ...
    def from_site_model(
        ...
        *,
        ...
        general_plan: dict[str, Any] | None,
        designations: dict[str, Any] | None,
        ...
        if property_type == "Unknown" and str(zoning.get("zoning") or "").upper().startswith("R-1"):
        ...
        allowed, allowed_reason = adu_eligibility(zoning, general_plan, property_type)
        ...
        return cls(
            ...
            flood=designations.get("flood") or {},
            geohazard=designations.get("geohazard") or {},
            historic=designations.get("historic") or {},
            wui=designations.get("wui") or {},
            heritage=designations.get("heritage_trees") or {},
        )
    ...
    return [
        ...
        _item(
            ...
            detail=(
                ...
                "Confirm structures are permitted at SJPermits.org."
            ),
        ...
        _item(
            ...
            detail=(
                "Resolve any active code enforcement complaint before ADU plans are accepted. "
                "Check the Code Complaints Map at SJPermits.org."
            ),
            source="Bulletin #210 Q3 / SJPermits.org Code Complaints Map",
        ),
    ]
...
def _part2_designations(ctx: ChecklistContext) -> list[dict[str, Any]]:
    flood = ctx.flood
    geohazard = ctx.geohazard
    historic = ctx.historic
    wui = ctx.wui

    return [
        _item(
            part=2, number=4,
            status=_tristate_status(flood.get("in_sfha")),
            question="Q4. Flood zones A, AE, AH, or AO?",
            detail=(
                flood.get("detail")
                or "Check SJPermits.org. If yes, plans must follow Bulletin #211. "
                "(Does not apply to zones D and X.)"
            ),
            source=flood.get("source") or "FEMA NFHL / San Jose Flood Hazard layer / SJPermits.org",
        ),
        _item(
            part=2, number=5,
            status=_tristate_status(geohazard.get("in_zone")),
            question="Q5. Geohazard or seismic hazards — geohazard, liquefaction, or landslide zone?",
            detail=(
                geohazard.get("detail")
                or "Check SJPermits.org ('Geohazard Zone' and 'Seismic Hazards'). "
                "If yes/landslide: Geologic Hazard Clearance required. "
                "If liquefaction + 2 or more units: Geologic Clearance required."
            ),
            source=geohazard.get("source") or "San Jose PLN Land Designations layer 31",
        ),
        _item(
            part=2, number=6,
            status=_tristate_status(historic.get("on_inventory")),
            question="Q6. Historic property — on City Historic Resources Inventory or CA Historical Resources list?",
            detail=(
                historic.get("detail")
                or "If yes + City Development Standards: simplified design standards per 20.80.175(E). "
                "If yes + State Standards: no additional historic standards."
            ),
            source=historic.get("source") or "San Jose HRI layers 406 + 408",
        ),
        _item(
            part=2, number=7,
            status=_tristate_status(wui.get("in_wui")),
            question="Q7. Wildland-Urban Interface (WUI) zone?",
            detail=(
                wui.get("detail")
                or "If yes, construction must comply with all WUI Fire Conformance Policy requirements."
            ),
            source=wui.get("source") or "San Jose WUI layer / USDA WUI",
        ),
        _item(
        ...
        _item(
            ...
            detail=(
                "Check pool permits at SJPermits.org. If a pool was demolished, "
                "submittal must include pool demolition documents (plot plan per Bulletin #289) "
                "or a geotech report + foundation design."
            ),
            source="Bulletin #210 Q9 / SJPermits.org permit history",
        ),
    ]
...
_TYPE_SELECTOR_DETAILS = {
    "detached": (
        ...
        "Must be sited in the rear yard or >= 45 ft from the front property line (City Standards). "
...
def _part3_development_standards(ctx: ChecklistContext) -> list[dict[str, Any]]:
    ...
    items.append(_item(
        part=3, number=10,
        status="pass" if ctx.allowed else "fail",
        question="Q10. Zoning / General Plan permits an ADU here",
        detail=(
            f"{ctx.allowed_reason} "
            f"Zoning: {ctx.zoning.get('zoning') or 'unknown'}; "
            f"GP: {ctx.general_plan.get('gp_designation') or 'unknown'}."
        ),
        source="San Jose Zoning layer 401 + General Plan layer 404",
    ))
...
def _part5_misc(ctx: ChecklistContext) -> list[dict[str, Any]]:
    ...
    heritage_present = ctx.heritage.get("has_heritage_tree")
    jadu_owner_occ = (
    ...
    return [
        _item(
            part=5, number=22,
            status="verify" if heritage_present in (True, None) else "pass",
            question="Q16. Tree removal — will the ADU require removal of a heritage tree?",
            detail=(
                ctx.heritage.get("detail")
                or "View the City Heritage Tree List. If yes, visit sanjoseca.gov/TreePermit."
            ),
            source=ctx.heritage.get("source")
                or "San Jose Heritage Trees layer 511 / sanjoseca.gov/TreePermit",
        ),
        _item(
            part=5, number=23,
...
def build_checklist(
    ...
    *,
    ...
    general_plan: dict[str, Any] | None = None,
    designations: dict[str, Any] | None = None,
